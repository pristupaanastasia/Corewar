...
t_car	*ft_add(t_car *car)
{
	...
	arg = malloc(sizeof(int) * 3);
	arg = read_arg(arg,g_arena[car->pc + 1],4);
	if (g_arena[car->pc + 2] > 0 && g_arena[car->pc + 2] <= REG_NUMBER && g_arena[car->pc + 3] > 0 && g_arena[car->pc + 3] <= REG_NUMBER && g_arena[car->pc + 4] > 0 && g_arena[car->pc + 4] <= REG_NUMBER && arg[0] == 1 && arg[1] == 1 && arg[2] == 1)
	{
